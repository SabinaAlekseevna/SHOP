/**
 * Подключение к MySQL для окна приложения "Платья".
 * @author SabinaTanya
 */
package MySQL;
 ...
 */
public class DressMysqlConnect extends MySQLConnect{
    static Logger logger = LoggerFactory.getLogger(DressMysqlConnect.class);
    /**
    ...
    public static ObservableList<Product> getDataProduct(){
        ...
        try {
            PreparedStatement ps = con.prepareStatement("select * from shop_clothes where name_product = 'Платье' order by size_product ");
